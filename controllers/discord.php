...
function init($redirect_url, $client_id, $client_secret, $bot_token = null)
{
    ...
    $_SESSION['discord_access_token'] = $results['access_token'];
}
...
function get_user($email = null)
{
    ...
    $headers = array('Content-Type: application/x-www-form-urlencoded', 'Authorization: Bearer ' . $_SESSION['discord_access_token']);
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, $url);
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
    ...
    curl_close($curl);
    $results = json_decode($response, true);
    $_SESSION['discord_user'] = $results;
    $_SESSION['discord_username'] = $results['username'];
    $_SESSION['discord_discrim'] = $results['discriminator'];
    $_SESSION['discord_user_id'] = $results['id'];
    $_SESSION['discord_user_avatar'] = $results['avatar'];
    # Fetching email 
    if ($email == True) {
        $_SESSION['discord_email'] = $results['email'];
    }
}
...
{
    $data = json_encode(array("access_token" => $_SESSION['discord_access_token']));
    $url = $GLOBALS['base_url'] . "/api/guilds/$guildid/members/" . $_SESSION['discord_user_id'];
